"""TopK Sparse Autoencoder implementation for neural network interpretation.

This module implements a TopK Sparse Autoencoder that constrains the number of
active neurons during reconstruction, promoting sparsity and interpretability.
"""
...
class TopKSAE(nn.Module):
    """TopK Sparse Autoencoder for learning sparse representations.
    
    The TopK SAE constrains the number of active neurons to exactly k during
    the forward pass, promoting sparse and interpretable representations.
    
    Args:
        input_dim: Dimension of input activations
        hidden_dim: Dimension of the hidden layer (typically much larger than input_dim)
        k: Number of top activations to keep (sparsity parameter)
    ...
    Example:
        >>> sae = TopKSAE(input_dim=512, hidden_dim=2048, k=64)
    ...
    def apply_topk(self, hidden: Tensor) -> Tuple[Tensor, Tensor]:
        """Apply TopK sparsity to hidden activations.
        ...
        topk_values, topk_indices = torch.topk(
            hidden, k=self.k, dim=-1, largest=True, sorted=False
        )
    ...
    def forward(
        ...
        """Forward pass through the TopK SAE.
    ...
    @classmethod
    def load_checkpoint(cls, filepath: str, device: Optional[str] = None) -> "TopKSAE":
