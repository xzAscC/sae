#!/usr/bin/env python3
"""Main training script for Absolute TopK SAE on a concept dataset.

This script demonstrates how to train an Absolute TopK Sparse Autoencoder on a concept-based dataset.

Usage:
    python scripts/train_absolute_topk_sae.py --config configs/gemma2_2b.yaml
    python scripts/train_absolute_topk_sae.py --quick-test  # For quick testing
"""
...
from topk_sae.models.absolute_topk_sae import AbsoluteTopKSAE
from topk_sae.data.concept_data_loader import ConceptDatasetLoader
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Train Absolute TopK SAE on a concept dataset"
    )
...
def prepare_data(
    ...
    data_loader = ConceptDatasetLoader(
        url=config.data.dataset_name, # Using dataset_name for the URL
...
def create_model(config: FullConfig, activation_dim: Optional[int] = None) -> AbsoluteTopKSAE:
    """Create Absolute TopK SAE model.
...
def create_optimizer(
    model: AbsoluteTopKSAE, config: FullConfig
...
def main() -> None:
    ...
    logging.info("Starting Absolute TopK SAE training")
